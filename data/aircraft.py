...
class Aircraft(Dataset):
    ...
    def init(root):
        ...
        images_train = pd.read_csv(os.path.join(root, 'fgvc-aircraft-2013b/data/images_variant_trainval.txt'),
                                   names=['filepath'])
        images_test = pd.read_csv(os.path.join(root, 'fgvc-aircraft-2013b/data/images_variant_test.txt'),
                                  names=['filepath'])
        train_images = []
        test_images = []
        ...
        for i in range(len(train_images)):
            images_train.append(
                [img_id_train[i], 'data/images/' + train_images[i], label_list_train[i]])
        ...
        for i in range(len(test_images)):
            images_test.append(
                [img_id_test[i], 'data/images/' + test_images[i], label_list_test[i]])
    ...
    def __getitem__(self, idx):
        ...
        return path, img, self.targets[idx], idx
